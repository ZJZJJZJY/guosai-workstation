...
以下 nature-* 系列技能已嵌入本工作流，在对应阶段自动联动，无需手动调用：

| 阶段 | 联动的 Skill | 用途 |
|------|-------------|------|
| 第二阶段·画图 | `nature-figure` | 生成 Nature 期刊标准的多面板科研图表（SVG/PDF/TIFF） |
| 第三阶段·英文结构 | `nature-writing` | 搭建 Nature 风格摘要、引言、结论框架 |
| 第三阶段·英文润色 | `nature-polishing` | 将中式英文打磨为 Nature 级别学术英文 |
| 第三阶段·引用 | `nature-citation` | 自动匹配 Nature/CNS 系列参考文献 |
| 答辩准备 | `nature-paper2ppt` | 从论文生成学术 PPT |

**触发规则**：当 math-modeling 进入对应阶段时，使用 Skill 工具调用上述 skill，确保每个阶段产出符合 Nature 期刊标准。
...
- 绘制图表进行数据可视化：**使用 Skill 工具调用 `nature-figure`，生成符合 Nature 期刊标准的多面板科研图表**（SVG/PDF 矢量输出，300+ dpi）。nature-figure 会先询问 Python 或 R，选 Python 即可与项目代码保持一致
- 创建README文档说明项目
...
- 检查用户是否提供论文模板
- **论文结构搭建**：使用 Skill 工具调用 `nature-writing`，从中文草稿搭建 Nature 风格摘要、引言、结果叙事、讨论、结论等章节结构
- **英文润色**：使用 Skill 工具调用 `nature-polishing`，将中式英文打磨为 Nature 级别学术英文
- **参考文献**：使用 Skill 工具调用 `nature-citation`，为正文段落自动匹配 Nature/CNS 系列参考文献
- 使用 docx skill 或默认模板（`references/论文模板.docx`）生成标准的 .docx 格式论文
- 避免死板罗列，使用段落式自然陈述
- 完成所有标准章节
...
**执行任务**：将论文转化为学术 PPT 供答辩或组会使用。

执行步骤：

- 使用 Skill 工具调用 `nature-paper2ppt`，从论文/摘要/图片生成 Nature 风格中文 PPTX
- PPT 包含论文核心：问题背景、模型方法、关键图表、结果分析、结论
- 自动提取论文中的图表并放入对应的 slide

**产出文件**：

- `答辩.pptx` - 数学建模答辩/汇报 PPT（.pptx 格式）
